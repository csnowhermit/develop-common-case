 ...
 */
public class JMSPersistTopicConsumer03 {
    public static void main(String[] args) {
        ...
        try {
            ...
            connection.setClientID("consumer-03");
            connection.start();

            Session session = connection.createSession(Boolean.TRUE, Session.AUTO_ACKNOWLEDGE);

            Topic destination = session.createTopic("myTopic");    //创建目的地
            MessageConsumer messageConsumer = session.createDurableSubscriber(destination, "consumer-03");    //创建消息的持久化订阅
